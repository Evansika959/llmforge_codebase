...
Taken: the whole project, 211 files, including the C kernels under `src/`, the export and parity tooling
...
comments, docstrings, error strings, report text, environment names and lock-file names were rewritten
to match. No numeric path was touched, so the measurements in `assets/device/pixel_watch5` still
describe these files. Fifteen of the changed files are recorded in the measurement manifests, whose digests were recomputed for the released tree with the
