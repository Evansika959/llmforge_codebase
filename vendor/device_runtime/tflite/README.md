...
Convert the custom LLMForge models to LiteRT (TFLite) for on-device Android, targeting the
...
Conversion starts from the **PyTorch** `GPT` (`$HOME/LLMForge`) + checkpoint — NOT the
`.rlm`/`.bin` C artifacts. This track is independent of the C engines under `src/`.

## Env (conversion workstation — this Linux box)

Isolated conda env `tflite` (kept separate from `llmforge_bridge`):
```bash
...
- `parity.py` — acceptance gate. `inspect` a `.tflite`'s signatures/tensors; `generate` greedy tokens
  to diff against `llmforge_bridge/ref_dump.py` (token-exact for fp32/fp16; track-then-diverge for int8).
