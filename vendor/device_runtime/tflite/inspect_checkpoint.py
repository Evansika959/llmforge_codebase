"""
Inspect a LLMForge checkpoint for the TFLite/LiteRT conversion (Phase 0).

Dumps the GPTConfig and the parameter name -> shape/dtype table so we can map the
LLMForge `GPT` state_dict onto the litert-torch Generative-API blocks (Phase 1),
and validate that a directory really is the smollm2_135M source checkpoint.

Usage:
  python tflite/inspect_checkpoint.py <ckpt_dir_or_ckpt.pt> [--llmforge $HOME/LLMForge]

Runs with plain torch (no litert-torch needed) — use the `llmforge_bridge` or `tflite` conda env.
...
def _add_llmforge_to_path(llmforge):
    # ckpt.pt may pickle references to LLMForge classes (GPTConfig, etc.); make them importable.
    for cand in [llmforge, "$HOME/LLMForge", "$HOME/LLMForge"]:
...
def main():
    ...
    ap.add_argument("--llmforge", default="$HOME/LLMForge",
                    help="LLMForge source dir (for unpickling class refs)")
    args = ap.parse_args()

    _add_llmforge_to_path(args.llmforge)
    ...
    if cfg is None:
        ...
        print(f"  (+{len(cfg_dict)} total config fields; use --llmforge's gpt_conf.py for the rest)")
