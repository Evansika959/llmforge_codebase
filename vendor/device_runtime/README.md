...
> **LLMForge fork.** This fork adds tooling to run LLMForge models (RoPE, GeGLU, RMSNorm,
> GPT-2 BPE) in this C engine, including on Android. See [`llmforge_bridge/`](llmforge_bridge/)
> and [`doc/llmforge_to_llama2c.md`](doc/llmforge_to_llama2c.md). An example model lives under
> [`models/smollm2_135M/`](models/smollm2_135M/). The tokenizer is committed, but the roughly
> 138 MB of weights are not, so export them from a supernet checkpoint with
> [`llmforge_bridge/export_llmforge_hetero.py`](llmforge_bridge/export_llmforge_hetero.py). Then:
> `make runqllmforge && ./runqllmforge models/smollm2_135M/smollm2_135M.q8.bin -g models/smollm2_135M/tokenizer_gpt2.bin -i "Once upon a time" -t 0.8 -p 0.9 -n 128`
