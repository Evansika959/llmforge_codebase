...
Tooling to export a model trained in [LLMForge](https://github.com/) and run it in this
pure-C engine (`src/run.c` / `src/runq.c`), including on Android. Full design, validation, and gate
results: [`../doc/llmforge2c.md`](../doc/llmforge2c.md).

## Files
- `export_llmforge.py` — LLMForge `ckpt.pt` → llama2.c `.bin` (`--version 0` fp32 for src/run.c,
  `--version 2` Q8_0 for src/runq.c). Validates the supported "Tier-1" architecture subset (uniform dims).
- `export_llmforge_hetero.py` — LLMForge `ckpt.pt` → `.rlm` (per-layer format) for **NSGA models with
  infinite-head attention**: heterogeneous per-layer dims, `infinite`/`identity` attention, GQA, peri-LN.
  `--version 1` fp32 (`make runllmforge` → `./run_llmforge model.rlm ...`, token-exact vs PyTorch);
  `--version 2` Q8_0 int8, ~4× smaller (`make runqllmforge` → `./runq_llmforge model.q8.rlm ...`).
  Design + parity results: [`../doc/llmforge_hetero_infinite.md`](../doc/llmforge_hetero_infinite.md).
...
- `run_live.sh` — live inference on the bundled model; interactive REPL or one-shot. Streams tokens.
  `llmforge_bridge/run_live.sh` (type prompts) or `llmforge_bridge/run_live.sh "Once upon a"`.
...
- `prove_parity.sh` — end-to-end proof the C engine == the original PyTorch model, token-for-token.
  `llmforge_bridge/prove_parity.sh [CKPT_DIR] [PROMPT] [N_STEPS]`.
...
```bash
# from repo root
python llmforge_bridge/export_llmforge.py <ckpt_dir> smollm2_135M_v2.bin --version 2
