/* Inference for HETEROGENEOUS LLMForge models in pure C.
 *
 ...
 * llmforge_bridge/export_llmforge_hetero.py. See doc/llmforge_hetero_infinite.md.
 *
...
// RMSNorm WITHOUT epsilon, matching LLMForge: rms = ||x||/sqrt(d); y = x/rms * gain
...
void error_usage() {
    fprintf(stderr, "Usage:   run_llmforge <model.rlm> [options]\n");
    fprintf(stderr, "Example: run_llmforge model.rlm -g tokenizer_gpt2.bin -i \"Once upon a time\" -t 0.8\n");
