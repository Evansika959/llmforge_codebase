/* Q8_0 inference for HETEROGENEOUS LLMForge models in pure C.
 *
 * Int8 (Q8_0 group-quantized) variant of run_llmforge.c: matmul weights are stored int8 with
 * per-group fp32 scales (norms stay fp32), roughly quartering model size for on-device use.
 * Reads version-2 `.rlm` files from llmforge_bridge/export_llmforge_hetero.py --version 2.
 * See doc/llmforge_hetero_infinite.md. Same architecture support as run_llmforge.c
...
void read_checkpoint(char* path, Transformer* t) {
    ...
    int version = hi[1];
    if (version != 2) { fprintf(stderr, "runq_llmforge needs a version-2 (Q8_0) .rlm; got version %d (use run_llmforge for fp32)\n", version); exit(EXIT_FAILURE); }
    Config* c = &t->config;
    c->n_layer    = hi[2];
    c->n_embd     = hi[3];
    c->vocab_size = hi[4];
...
void rmsnorm(float* o, float* x, float* weight, int size) {
    ...
    ss /= size;
    ss = 1.0f / sqrtf(ss);          // NO eps (matches LLMForge RMSNorm)
...
void error_usage() {
    fprintf(stderr, "Usage:   runq_llmforge <model.q8.rlm> [options]\n");
    fprintf(stderr, "Example: runq_llmforge model.q8.rlm -g tokenizer_gpt2.bin -i \"Once upon a time\" -t 0.8\n");
