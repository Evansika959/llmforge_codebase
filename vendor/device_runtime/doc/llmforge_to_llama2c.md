# Design: Run a LLMForge Tier-1 model on llama2.c (Android target)
...
First target model: `smollm2_135M` from `$HOME/Evo_GPT_checkpoints_backup/smollm2_135M`

## Goal

Run a LLMForge-trained model in the pure-C llama2.c inference engine (`run.c` / `runq.c`)
...
- RoPE: run.c (lines 264-279) rotates adjacent pairs with exponent (i%head_size)/head_size =
  interleaved/GPT-J, identical to LLMForge RotaryEmbedding (even/odd adjacent pairs, theta 10000).
  Frequencies match (run.c pair k=i/2 ↔ exponent 2k/head_size ↔ inv_freq[k]=10000^(-k/(head_size/2))).
- GQA: run.c uses `h / kv_mul` block sharing (kv_mul=3); LLMForge uses block (repeat_interleave)
...
- SwiGLU branch roles: run.c w1=gated (activation), w3=value, w2=down. LLMForge c_fc_in1=gated,
...
2. **RMSNorm eps.** run.c adds +1e-5 inside sqrt; LLMForge adds none. Negligible (~1e-5 relative),
...
4. Byte-format must match the EXACT readers in this repo. IMPORTANT: this repo's `run.c` is a
   ...
   tensors straight from the strip-prefixed LLMForge state_dict per the mapping in step 3.
...
NOTE (verified): LLMForge `sample.py` has NO logit/token-ID dump and NO greedy/argmax mode
...
- New script `ref_dump.py` (lives alongside our work, imports LLMForge model):
  - Add `$HOME/LLMForge` to sys.path; `from model import GPT` and `from gpt_conf import GPTConfig`.
...
```
# export
python llmforge_bridge/export_llmforge.py $HOME/Evo_GPT_checkpoints_backup/smollm2_135M smollm2_135M_v0.bin --version 0
python llmforge_bridge/export_llmforge.py $HOME/Evo_GPT_checkpoints_backup/smollm2_135M smollm2_135M_v2.bin --version 2
# build (GELU)
gcc -O3 -DACT_GELU -o run_gelu  src/run.c  -lm
gcc -O3 -DACT_GELU -o runq_gelu src/runq.c -lm
# validate parity
printf "464 2068 7586 21831\n" > ids.txt
python llmforge_bridge/ref_dump.py $HOME/Evo_GPT_checkpoints_backup/smollm2_135M ids.txt 32 2>/dev/null > ref_out.txt
...
python llmforge_bridge/export_gpt2_tokenizer.py tokenizer_gpt2.bin
