...
class AndroidBackend:
    ...
    def __call__(self, rd, proposal, protocol, config):
        ...
        from .storage import digest
        if protocol['kernel_id'] != digest(ROOT/'src/runq_llmforge.c'):
        ...
        with locked(lock):
            ...
            if missing:
                ...
                for name in ['temp_device_ckpt.pt','temp_device_model.q8.rlm','runq_llmforge_device','power_sampler_device','temp_trace_raw.csv','temp_infer_log.txt']:
                    if (ROOT/name).exists():
