...
def ensure_compiled_and_pushed(adb_base):
    ...
    adb_push_with_retry(adb_base, "runq_llmforge_device", "/data/local/tmp/runq_llmforge")
    subprocess.run(adb_base + ["shell", "chmod +x /data/local/tmp/runq_llmforge"], capture_output=True, check=True)
    if os.path.exists("runq_llmforge_device"):
        os.remove("runq_llmforge_device")
...
def export_model():
    export_script = os.path.join(REPO_ROOT, "llmforge_bridge", "export_llmforge_hetero.py")
