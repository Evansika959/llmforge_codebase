...
`evaluate_layer` decomposes a layer with infinite-head attention into seven GEMMs, and a SwiGLU MLP adds an eighth for its gate projection. A GEMM maps K input channels to N output channels over L rows, and Timeloop maps each GEMM onto the substrate.
...
**Before 2026-09-15.** The backend costed every layer like the first active layer, used the query and key head dimension in place of the value head dimension, combined on-chip energy with DRAM-bound cycles, scaled area with SRAM alone, and re-mapped cached shapes on every call. It also priced two MLP GEMMs and packed two MLP weight matrices into WMEM for SwiGLU layers. On the uniform SmolLM2 grids, adding `MLP_gate` and its weights raises energy per token by a median 24 percent at SmolLM2-135M and 33 percent at SmolLM2-360M, raises TPOT by 11 and 15 percent, and changes the selected chip of most architectures. rDXE results from before that date are superseded.
