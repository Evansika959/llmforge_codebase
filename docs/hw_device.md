...
The runtime is `nanollmforge.c`, a separate C inference engine pinned at commit `d517dd8234ba723e7ec394d708a0347218381e9f`. Its URL is withheld for anonymous review. The kernel identity in the dataset protocol is the SHA-256 of `src/runq_reallm.c` at that commit, and all 70 source hashes recorded in `provenance/contract.json` match that commit. Set `LLMFORGE_DEVICE_RUNTIME` to a checkout of it. Building needs an Android NDK through `NDK` or `ANDROID_NDK_HOME`, and measuring needs `adb`.
