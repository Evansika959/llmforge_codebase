...
class ActiveLearningTests(unittest.TestCase):
    ...
    def test_dry_step_does_not_access_hardware_and_freezes_proposal(self):
        self.init()
        with patch('llmforge.hw.device.prediction.active_learning.hardware.preflight',side_effect=AssertionError('No ADB allowed')):
    ...
    def test_initial_checkpoint_is_loaded_without_refitting_predictor(self):
        ...
        with patch('llmforge.hw.device.prediction.active_learning.engine.fit_dataset',side_effect=AssertionError('Do not refit initial checkpoint')):
            advance(self.root)
    ...
    def test_sampling_does_not_reward_predicted_performance(self):
        ...
        with patch('llmforge.hw.device.prediction.active_learning.sampling.committee_predictions',
    ...
    def test_preflight_refuses_charger_low_battery_and_busy_device(self):
        for charging,level,busy,expected in [('true',80,'','Unplug'),('false',30,'','charge'),
            ...
            with self.subTest(expected=expected),patch('llmforge.hw.device.prediction.active_learning.hardware.subprocess.run',side_effect=run):
    ...
    def _adapter_resume(self, anchors, strict=False):
        ...
        with patch.object(hardware,'RUNTIME',fake_repo),patch.object(hardware,'WORK',Path(self.temp.name)/'work'),\
             patch.object(hardware,'preflight',return_value=snapshot),\
    ...
    def test_migrate_pending_batch_preserves_source_and_resumes_without_anchors(self):
        ...
        with patch('llmforge.hw.device.prediction.active_learning.hardware.preflight',side_effect=AssertionError('No ADB')):
    ...
    def test_migration_refuses_unacknowledged_or_unrelated_source_changes(self):
        ...
        changed[migration.source_key(Path(migration.__file__).with_name('quality.py'))]='changed'
