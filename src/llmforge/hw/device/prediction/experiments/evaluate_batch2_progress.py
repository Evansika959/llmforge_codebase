...
def main():
    ...
    parser.add_argument('--output', type=Path, default=OUTPUTS/'batch2_progress_632')
    parser.add_argument('--snapshot', type=Path, help='Reuse input_snapshot.json from a previous run.')
    args = parser.parse_args()
    out = args.output
    out.mkdir(parents=True, exist_ok=False)
    torch.set_num_threads(4)
    olddir = OUTPUTS/'surrogate_comparison_936'
    proposed = OUTPUTS/'proposed_physics_surrogate_936'
    oldmeta = json.loads((olddir / 'metadata.json').read_text())
    if args.snapshot:
        snapshot = json.loads(args.snapshot.read_text())
    else:
        paths = dict(batch1=Path(oldmeta['source']),
            batch2=SWEEP_OUTPUTS/'watch5_random_50M_150M_40C_decode32_batch2_results.csv',
            config1=SWEEP_CONFIGS/'watch5_random_1000_50M_150M_sweep.csv',
            config2=SWEEP_CONFIGS/'watch5_random_1000_50M_150M_sweep_batch2.csv')
    ...
    seen = set(json.loads((OUTPUTS/'state_signal_investigation_936/metadata.json').read_text())['batch2_ids'])
    ...
    lines += ['', '## Interpretation limits', '',
        ...
        f'python -m llmforge.hw.device.prediction evaluate-batch2-progress --snapshot {out}/input_snapshot.json --output runs/device/batch2_progress_repeat', '```']
