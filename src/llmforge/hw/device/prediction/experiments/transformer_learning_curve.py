...
def main():
    ...
    ap.add_argument('--output',default=str(OUTPUTS/'transformer_learning_curve_936'))
    args=ap.parse_args(); out=Path(args.output);out.mkdir(parents=True,exist_ok=False)
    src=SWEEP_OUTPUTS/'watch5_random_50M_150M_40C_decode32_results_clean.csv'
    ...
    lines+=['','![Learning curves](learning_curve.png)','',
        ...
        '', '## Reproduce','', '```bash',
        'python -m llmforge.hw.device.prediction transformer-learning-curve --output runs/device/transformer_learning_curve_repeat','```']
