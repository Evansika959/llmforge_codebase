...
def main():
    ...
    parser.add_argument('--output',default=str(OUTPUTS/'xgboost_physics_priors_936'))
    args=parser.parse_args()
    out=Path(args.output)
    out.mkdir(parents=True,exist_ok=False)
    previous=OUTPUTS/'surrogate_comparison_936'
    ...
    cfg_path=SWEEP_CONFIGS/'watch5_random_1000_50M_150M_sweep.csv'
    ...
    lines+=['','## Feature definitions','',
        ...
        '', '## Reproduce','', '```bash', 
        'python -m llmforge.hw.device.prediction compare-physics-priors --output runs/device/xgboost_physics_priors_repeat','```']
