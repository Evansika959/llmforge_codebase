...
def main():
    ap=argparse.ArgumentParser();ap.add_argument('--output',default=str(OUTPUTS/'state_signal_investigation_936'))
    args=ap.parse_args();out=Path(args.output);out.mkdir(parents=True,exist_ok=False)
    rows,sourcehash=load(SWEEP_OUTPUTS/'watch5_random_50M_150M_40C_decode32_results_clean.csv')
    cfg1,_=load(SWEEP_CONFIGS/'watch5_random_1000_50M_150M_sweep.csv')
    cfg2,_=load(SWEEP_CONFIGS/'watch5_random_1000_50M_150M_sweep_batch2.csv')
    cfg={r['config_id']:r for r in cfg1+cfg2}
    old=OUTPUTS/'surrogate_comparison_936'
    original=json.loads((old/'metadata.json').read_text());assert sourcehash==original['source_sha256']
    b2raw,b2hash=load(SWEEP_OUTPUTS/'watch5_random_50M_150M_40C_decode32_batch2_results.csv')
    ...
    conditioned=OUTPUTS/'xgboost_error_diagnosis_936'
    ...
    lines+=['','## What this can and cannot establish','',
        ...
        '', '## Reproduce','', '```bash',
        'python -m llmforge.hw.device.prediction investigate-state-signal --output runs/device/state_signal_investigation_repeat','```']
