...
def main():
    ...
    with (SWEEP_OUTPUTS/'watch5_random_50M_150M_40C_decode32_results_clean.csv').open() as f: rows=list(csv.DictReader(f))
    ...
    lines+=['','![Learning curve](learning_curve.png)','',
        ...
        '', '## Reproduce','', '```bash',
        'python -m llmforge.hw.device.prediction diagnose-surrogate-error --output runs/device/xgboost_error_diagnosis_repeat',
        'python -m llmforge.hw.device.prediction report-error-diagnosis runs/device/xgboost_error_diagnosis_repeat','```']
