...
from .engine import initialize, source_hashes, source_key, verify_workspace
from .migration import _source_state
from .quality import result_metrics
from .sampling import unseen_candidates
from .storage import atomic_json, atomic_bundle, digest, locked, portable_path, read_json

ALLOWED = {source_key(Path(__file__).resolve().parent/f'{name}.py') for name in
...
def switch_to_dynamic(source, destination, legacy_snapshot, reason, accept_source_changes=False,
    ...
    with locked(source/'RUNNING.lock'):
        ...
        def portable(mapping):
            # Recorded provenance keys are relative to the directory that holds the workspaces.
            return {portable_path(path,source.parent):sha for path,sha in mapping.items()}
        legacy = {}
        ...
        document = dict(schema_version=1,protocol=protocol,observations=converted,
            source_hashes=portable(sources),energy_transition=dict(source_workspace=source.name,
        ...
        atomic_json(marker,dict(source=source.name,kind='gross_to_dynamic'))
        ...
        record = dict(created_utc=datetime.now(timezone.utc).isoformat(),reason=reason.strip(),
            source_workspace=source.name,source_changes=changes,source_file_hashes=portable(sources),
