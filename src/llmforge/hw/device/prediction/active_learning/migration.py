...
from .engine import source_hashes, source_key, verify_workspace
from .storage import atomic_json, digest, locked, read_json, write_configs

ACTIVE_LEARNING_ROOT = Path(__file__).resolve().parent
POLICY_FILES = {source_key(ACTIVE_LEARNING_ROOT/f'{name}.py')
...
def disable_anchors(source, destination, reason, accept_source_changes=False):
    ...
    with locked(source/'RUNNING.lock'):
        ...
        atomic_json(marker,dict(source=source.name,reason=reason.strip()))
        ...
        record = dict(created_utc=datetime.now(timezone.utc).isoformat(),source=source.name,
...
"""
    ...
    allowed = POLICY_FILES | {source_key(ACTIVE_LEARNING_ROOT/'worker.py'),source_key(ACTIVE_LEARNING_ROOT/'thermal.py')}
    with locked(source/'RUNNING.lock'):
        ...
        destination.mkdir(parents=True,exist_ok=False)
        marker = destination/'MIGRATION_IN_PROGRESS.json'
        atomic_json(marker,dict(source=source.name,reason=reason))
        shutil.copytree(source,destination,dirs_exist_ok=True,ignore=shutil.ignore_patterns('RUNNING.lock'))
        for name,sha in manifest.items():
            if digest(destination/name)!=sha:
        ...
        atomic_json(destination/'state.json',state)
        atomic_json(audit/'record.json',dict(source=source.name,created_utc=datetime.now(timezone.utc).isoformat(),
