...
def main():
    ...
    from llmforge.hw.device.measurement.sweep import run_sweep_configs as legacy
    if strict_temperature:
    ...
    class PreservingOS:
        ...
        def remove(self,path):
            if Path(path).name in ('temp_trace_raw.csv','temp_infer_log.txt') and current['folder'] is not None:
