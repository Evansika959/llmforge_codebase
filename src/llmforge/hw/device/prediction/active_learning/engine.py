...
from llmforge.hw.device.runtime import EXPORTER, KERNEL_SOURCE, RUNTIME, SAMPLER_SOURCE

from ..config import DEVICE_ROOT, FEATURES, PACKAGE_ROOT, PREDICTION_ROOT, SWEEP_ROOT
...
from .storage import atomic_json, atomic_bundle, read_json, write_configs, digest, locked, portable_path
...
def source_key(path):
    """Contract key of a package source file: its path from the source root."""
    return Path(path).resolve().relative_to(PACKAGE_ROOT.parent).as_posix()


def source_hashes():
    """Hashes of the predictor code, the sweep harness and the pinned runtime sources.

    A runtime file that is not checked out is recorded as None. A workspace initialized without the
    runtime therefore refuses to continue once the runtime appears, instead of mixing identities.
    """
    files = [p for p in PREDICTION_ROOT.rglob('*.py') if 'tests' not in p.relative_to(PREDICTION_ROOT).parts]
    files += [SWEEP_ROOT/'run_sweep_configs.py',SWEEP_ROOT/'generate_watch5_random_50m_150m.py',DEVICE_ROOT/'runtime.py']
    hashes = {source_key(p):digest(p) for p in files}
    for relative in [KERNEL_SOURCE,SAMPLER_SOURCE,EXPORTER]:
        path = RUNTIME/relative
        hashes['runtime/'+relative] = digest(path) if path.is_file() else None
...
def initialize(workspace, dataset, candidates=None, settings=None, initial_model=None):
    ...
    if initial_bundle is not None:
        ...
        atomic_json(workspace/'initial_model_source.json',dict(path=portable_path(initial_model,workspace.parent),sha256=digest(initial_model)))
...
def _proposal(workspace, rd, dataset, config, number):
    ...
    if checkpoint.exists():
        ...
        model_origin = dict(path=portable_path(checkpoint,workspace),sha256=digest(checkpoint),loaded=True)
    ...
    with locked(workspace/'RUNNING.lock'):
        ...
        while state['completed_rounds'] < stop:
            ...
            if not (rd/'ingested.json').exists():
                ...
                files = [result_path,rd/'schedule.csv',rd/'proposal.json']
                if (rd/'attempt_audit.json').exists():
                    files.append(rd/'attempt_audit.json')
                # Provenance keys are relative to the directory that holds the workspace.
                sources = {portable_path(p,workspace.parent):digest(p) for p in files}
